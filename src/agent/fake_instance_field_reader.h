...
#ifndef DEVTOOLS_CDBG_DEBUGLETS_JAVA_FAKE_INSTANCE_FIELD_READER_H_
#define DEVTOOLS_CDBG_DEBUGLETS_JAVA_FAKE_INSTANCE_FIELD_READER_H_
...
// Implements "InstanceFieldReader" interface exposing preset data. This class
// is intended for unit tests only. It correctly implements the "Clone" method,
// which is something gMock doesn't handle.
class FakeInstanceFieldReader : public InstanceFieldReader {
 public:
  FakeInstanceFieldReader(const std::string& name, const JSignature& signature,
                          const JVariant& expected_value)
      : name_(name), signature_(signature), expected_value_(expected_value) {}

  static std::unique_ptr<InstanceFieldReader> Create(
      const std::string& name, const JSignature& signature,
      const JVariant& expected_value) {
    return std::unique_ptr<InstanceFieldReader>(
        new FakeInstanceFieldReader(name, signature, expected_value));
  }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(FakeInstanceFieldReader);
};

}  // namespace cdbg
}  // namespace devtools

#endif  // DEVTOOLS_CDBG_DEBUGLETS_JAVA_FAKE_INSTANCE_FIELD_READER_H_
