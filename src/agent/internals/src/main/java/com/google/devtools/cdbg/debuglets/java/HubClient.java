 ...
 *
 * <p>This interface is implemented separately for each communication protocol and
 * integration test. All methods throw {@link Exception} in case of failure because
 * specific error type depends on the communication protocol library.
 *
 * <p>HubClient interface does not prescribe specific data format to serialize
 * breakpoints. The native code serializes breakpoint into the format it recognizes.
 * It is up to the configuration to make sure that the receiving end recognizes
 * the data format.
 *
...
public interface HubClient {
   ...
   *
   * <p>This method will only throw exception if the request fails due to a transient error
   * (such as destination unreachable). If the request fails with permanent error, this
   * function will silently absorb it. Prime example of such a case is updating breakpoint
   * that's already in a final state.
   *
   * <p>The breakpoint ID is duplicated in breakpointId argument and inside the serialized
   * data. This is done for performance reasons, so that the implementation of {@link HubClient}
   * doesn't need to deserialize the breakpoint just to get an ID.
   *
