 ...
 *
 * Under Bazel, we are using the 'data' build attribute to specify test files
 * that should be included in the in the runfiles area of the test sandbox.
 * These are files the tests themselves need access to for opening and reading
 * as part of the test execution. Given these files end up in a sandboxed
 * environment where the base root path is not known, we leverage the Bazel
 * Runfiles utility to locate the file so we can obtain the full path.
 *
 * See:
