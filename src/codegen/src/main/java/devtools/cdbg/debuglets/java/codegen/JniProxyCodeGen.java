...
public class JniProxyCodeGen {
  ...
  public static class ProxyConstructor {
    ...
    /**
     * Returns true if the generated method allocates a subclass of this class and not necessarily
  ...
  public JniProxyCodeGen(File path, Class<?> cls, Config.ProxyClass config) {
    ...
    Comparator<Object> methodComparator =
    ...
    for (Config.ProxyMethod methodConfig : config.getMethods()) {
        ...
        for (Constructor<?> constructor : clsConstructors) {
          if ((methodConfig.getMethodSignature() != null)
              && !Type.getConstructorDescriptor(constructor)
                  .equals(methodConfig.getMethodSignature())) {
            continue;
          }

          constructors.add(
              new ProxyConstructor(
                  methodConfig.isSubclassConstructor(), constructor, constructors.size()));
          found = true;
        }
      } else {
      ...
      if (!found) {
        throw new RuntimeException(
            String.format(
                "Method %s (signature: %s) not found in class %s",
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (Config.ProxyClass javaClass : config.getClasses()) {
      JniProxyCodeGen gen =
          new JniProxyCodeGen(
              new File(args[1]), Class.forName(javaClass.getClassName()), javaClass);
      gen.generate();
    }
  }
}
